...
| `echelle inventory DRIVE_ROOT [--output DIR]` | Answers *which drive is this*: the curator's `*cycle*_Echelle_setup.xlsx` logbooks (days, date span, trigger delays, and every day not taken at 2.5 s), the observing day folders with their shot ranges and frame counts, the calibration folders, the free space, and whether the cubes would fit beside the data. Reads only; writes one `inventory/<volume>.json` under `--output` (default: this folder). A logbook needs `openpyxl`; without it the logbook is reported present-but-unread rather than skipped | Yes; run it the moment a drive is plugged in |
...
| `echelle drift geometry DATA_ROOT` | Answers *where does this light sit*: reads one mid-day frame per sampled day (`--every N`, default 6; `--every 0`, or a single day folder, asks the three-frame spread — first day, middle day, last day) and measures its order-band centres against each `--pattern` given, defaulting to the packaged 2024 and 2025 patterns. Prints the per-day table, writes `inventory/<volume>-pattern-survey.json`, and ends on `GEOMETRY_OK` or `GEOMETRY_ALARM` against the 1.5-row alarm for the **last** pattern given. Exit 0 inside the alarm, 2 past it, 3 when nothing could be measured | Yes; required before a bulk run over a failing wavelength audit |
